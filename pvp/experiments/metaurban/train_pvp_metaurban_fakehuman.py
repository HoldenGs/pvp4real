...
# Fix multiprocessing issues in WSL2
...
if __name__ == '__main__':
    ...
    parser.add_argument("--eval_freq", default=1000, type=int, help="Evaluate every N timesteps")
    parser.add_argument("--n_eval_episodes", default=5, type=int, help="Number of episodes per evaluation")
    parser.add_argument("--eval_start_seed", default=1000, type=int, help="Starting seed for evaluation scenarios")
    parser.add_argument("--eval_horizon", default=1500, type=int, help="Max steps per evaluation episode")
    parser.add_argument("--enable_eval", action="store_true", help="Enable periodic evaluations")
    ...
    config = dict(
        ...
        env_config=dict(
            use_render=True,           # enable rendering to visually see expert takeovers
            manual_control=False,      # remote actors won't override
            free_level=free_level,     # probability threshold for takeover
            disable_expert=False,      # enable expert now that we have a new one
    ...
    # ——— Also build the eval env (with fallback for WSL2 issues) ———
    eval_env = None
    if args.enable_eval:
        print("Warning: Periodic evaluation requested but may have issues in WSL2.")
        print("If you encounter multiprocessing errors, run without --enable_eval")
        print("and use a separate evaluation script instead.")
        
        try:
            def _make_eval_env():
                eval_env_config = dict(
                    use_render=False,           # no rendering during evaluation
                    manual_control=False,       # no manual control during evaluation
                    start_seed=args.eval_start_seed,  # different scenarios from training
                    horizon=args.eval_horizon,  # evaluation episode length
                )
                # Use HumanInTheLoopEnv for evaluation to avoid expert loading conflicts
                eval_env = HumanInTheLoopEnv(config=eval_env_config)
                eval_env = Monitor(env=eval_env, filename=str(trial_dir))
                return eval_env

            eval_env = SubprocVecEnv([_make_eval_env])
            print(f"Evaluation enabled: {args.n_eval_episodes} episodes every {args.eval_freq} steps")
        except Exception as e:
            print(f"Failed to create evaluation environment: {e}")
            print("Continuing without evaluation. Use a separate evaluation script for best results.")
            eval_env = None
    ...
    # Add evaluation parameters if evaluation is enabled
    if args.enable_eval:
        learn_kwargs.update(dict(
            eval_env=eval_env,
            eval_freq=args.eval_freq,
            n_eval_episodes=args.n_eval_episodes,
            eval_log_path=str(trial_dir / "eval"),
        ))
